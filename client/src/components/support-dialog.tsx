            ...
            <div className="grid grid-cols-4 gap-2">
              {amounts.map((option) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all duration-200 relative overflow-visible",
                    selectedAmount === option.value
                      ? "border-primary bg-primary/5 text-primary"
                      : "border-transparent bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground",
                    option.value === 25 && selectedAmount !== 25 && "text-amber-500/80 hover:text-amber-600",
                    option.value === 25 && selectedAmount === 25 && "border-amber-500 text-amber-600 bg-amber-500/10"
                  )}
                >
                  {option.value === 25 && (
